...
<<<<<<< HEAD
mu_0 <- __ #enter hypothesized mean
sigma <- __
n <- sum(!is.na(spi_col)) #total number of teams
xbar <-
z <- __ #calculate z-critical value
=======
...
<figure style="text-align:center">
  <img src="./images/music.jpg" style="width:40%">
  <figcaption>Photo by <a href="https://unsplash.com/@sickhews?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Wes Hicks</a> on <a href="https://unsplash.com/photos/several-guitars-beside-of-side-table-MEL-jJnm7RQ?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
      </figcaption>
</figure>
...
<figure style="text-align:center">
  <img src="./images/videogames.jpg" style="width:40%">
  <figcaption>Photo by <a href="https://unsplash.com/@jeshoots?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">JESHOOTS.COM</a> on <a href="https://unsplash.com/photos/two-people-playing-sony-ps4-game-console-eCktzGjC-iU?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
      </figcaption>
</figure>
...
## Q9 — One-sample proportion using prop_test()

Now repeat the same test but we use a function that gives the z statistic directly in 1 line.
...
z_val <- as.numeric(res$statistic[1])
p_val <- as.numeric(res$p[1])
p_hat <- x / n
...
Same hypothesis as Q7–Q8, just give x, n, and p = 0.045.
:::
...
fail("The rstatix package is not available.")
}

exp_res <- rstatix::prop_test(
x = x,
n = n,
p = p0,
alternative = "two.sided"
...
exp_z <- exp_res$statistic[1]
exp_p <- exp_res$p[1]
exp_p_hat <- x / n

r <- .result
if (!is.numeric(r) || !all(c("p_hat","z","p_value") %in% names(r))) {
fail("Return c(p_hat=..., z=..., p_value=...).")
}

if (max(abs(r["p_hat"] - exp_p_hat),
abs(r["z"] - exp_z),
abs(r["p_value"] - exp_p)) < 1e-6) {
pass("Correct: you extracted z and p from rstatix::prop_test().")
} else {
fail("Recheck p-hat = x/n, z = statistic, and p_value = p.")
}
})
...
## Q10 — Get a signed Z from prop.test()

We use the same hypothesis as Q7–Q8:
...
prop.test() gives us a chi-square with 1 df. For 1 df, $\chi^2 = Z^2$
. Q8 took the square root and got the magnitude. Here we will also:

1) Get $\hat{p}$ from the data
2) Compare $\hat{p}$ to $p_0$
3) Give the Z a sign based on that
...
out <- prop.test(x = x, n = n, p = p0, alternative = "two.sided")

chisq_val <- as.numeric(out$statistic) #chi sq from prop.test

z_mag <- __ #magnitude of z

p_hat <- __
sign_z <- ifelse(p_hat - p0 >= 0, 1, -1) #figure out the sign

z_signed <- __

p_two <- out$p.value #two sided p value. this should match prop.test

c(
  p_hat   = as.numeric(p_hat),
  z       = as.numeric(z_signed),
  p_value = as.numeric(p_two)
)

```

::: {.hint exercise="q9_vgs_prop_rstatix"}
Use the same prop.test() call as Q7, then:

sqrt(out$statistic) for |z|

make it negative if p_hat < p0
:::
...
z_mag <- sqrt(chisq_val)

p_hat <- x / n
sign_z <- ifelse(p_hat - p0 >= 0, 1, -1)
z_signed <- sign_z * z_mag
...
exp_out <- prop.test(x = x, n = n, p = p0, alternative = "two.sided")
exp_p <- exp_out$p.value
exp_chisq <- as.numeric(exp_out$statistic)
exp_z_mag <- sqrt(exp_chisq)
p_hat <- x / n
sign_z <- ifelse(p_hat - p0 >= 0, 1, -1)
exp_z_signed <- sign_z * exp_z_mag

res <- .result
if (!is.numeric(res)) fail("Return c(p_hat=..., z=..., p_value=...).")
if (!all(c("p_hat","z","p_value") %in% names(res))) fail("Name your output p_hat, z, and p_value.")

if (abs(res["p_value"] - exp_p) < 1e-6 &&
abs(res["z"] - exp_z_signed) < 1e-6 &&
abs(res["p_hat"] - p_hat) < 1e-6) {
pass("Correct: derived signed z and p-value from prop.test().")
} else {
fail("Recheck: run the same prop.test, take sqrt(statistic), and set the sign from p̂ − p0.")
}
})
```

## Q11 — One-sample hypothesis test on the variance (σ² known under H₀)

We want to test whether the variance of Track.Score is equal to a target value. Suppose a platform claims that Track.Score has variance $\sigma_0^2 = 100$ (i.e. SD=10) We will test:

$(H_0: \sigma^2 = 100)$ vs
$(H_1: \sigma^2 \neq 100)$

This is a chi-square based variance test.


::: {.callout-note title="Info"}
For a one sample variance test $$\chi^2 = \frac{(n - 1)s^2}{\sigma_0^2} \sim \chi^2_{\,n - 1} \quad \text{under } H_0.$$ EnvStats::varTest() does this for you and gives the p-value.
:::
...
```{webr}
#| exercise: q10_trackscore_var_envstats
#| exercise.lines: 8
#| echo: false
...
out <- EnvStats::varTest(x,
sigma.squared = ＿＿,   # 100
alternative   = "two.sided")
...
::: {.hint exercise="q10_trackscore_var_envstatss"}
Use the Track.Score column, test against 100, two-sided.
:::

::: {.solution exercise="q10_trackscore_var_envstats"}
...
df <- tryCatch(read.csv("spotify-2024.csv"), error = function(e) NULL)
if (is.null(df)) fail("Couldn't read 'spotify-2024.csv'.")
if (!"Track.Score" %in% names(df)) fail("Need a 'Track.Score' column.")

x <- df$Track.Score

ok <- requireNamespace("EnvStats", quietly = TRUE)
if (!ok) fail("The EnvStats package is not available in this environment.")

exp_out <- EnvStats::varTest(x,
sigma.squared = 100,
alternative   = "two.sided")
exp_p <- exp_out$p.value

res <- .result
if (!is.numeric(res) || length(res) != 1L) fail("Print a single numeric p-value.")
if (abs(res - exp_p) < 1e-6) {
pass("Correct variance test with EnvStats::varTest().")
} else {
fail("Check sigma.squared = 100 and alternative = 'two.sided'.")
}
})
