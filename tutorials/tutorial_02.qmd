...
::: {.callout-note title="Info"}
skimr::skim() is like an enhanced summary(): it detects column types and prints type-specific stats (e.g., numeric vs. factor). Great to see NAs and distributions quickly. 
:::

```{webr}
#| exercise: ex_peng_skim
#| exercise.lines: 2
#| echo: false
# One expression that returns a skim data frame
```

::: {.hint exercise="ex_peng_skim"}
Call the package’s main function on the entire data frame; no assignments.
:::

::: {.solution exercise="ex_peng_skim"}
```{webr}
#| exercise: ex_peng_skim
#| solution: true
skimr::skim(penguins)
...
gradethis::grade_this({
  ...
  x <- .result
  if (!is.data.frame(x)) fail("Return the skim output data frame (no variable needed).")
  pass("✅ skim captured.")
})
```
