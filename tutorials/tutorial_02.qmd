...
webr::install(c("ggplot2", "gradethis"))
```
...
gradethis::grade_this({
  p <- .result
  if (!inherits(p, "ggplot")) fail("Return a ggplot object.")
  geoms <- vapply(p$layers, function(l) class(l$geom)[1], "")
  if (!any(grepl("GeomHistogram", geoms))) fail("Add a histogram with geom_histogram().")
  if (!any(grepl("GeomVline", geoms))) fail("Add the vertical lines with geom_vline().")
  pass("✅ Looks good: histogram + fences detected.")
})
```
