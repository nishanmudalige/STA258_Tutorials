...
ggplot2:: ＿ ( ＿ ,
  ggplot2::aes(x = reorder(factor( ＿ ), ＿ , FUN = median), y = ＿ )
) + ggplot2:: ＿ () #Boxplot command in ggplot
...
qs <- quantile(mtcars$mpg, ＿ )
lo <- qs[1] - 1.5*diff(qs); hi <- ＿
ggplot2::ggplot( ＿ , ggplot2::aes(mpg)) +
  ggplot2::geom_histogram(bins = 12) +
  ggplot2::geom_vline(xintercept = c(qs[1], ＿ , lo, hi))
...
## Q6 — ChickWeight (Day 21): grouped summary

::: {.callout-note title="Info"}
**ChickWeight** tracks chick body weight over time under different diets.  
Filter to **Time == 21** (final day), then group by **Diet**, summarising a row count and two key stats.
:::

```{webr}
#| exercise: ex_chickweight
#| exercise.lines: 10
#| echo: false
# One pipeline: filter Time==21 then summarise by Diet
```

::: {.hint exercise="ex_chickweight"}
Filter for the focal day (Time == 21), group_by(Diet), then summarise() a count and two numeric summaries; drop grouping in the output.
:::

::: {.solution exercise="ex_chickweight"}
```{webr}
#| exercise: ex_chickweight
#| solution: true
ChickWeight |>
...
gradethis::grade_this({
  ...
  x <- .result
  need <- c("Diet","n","weight_mean","weight_sd")
  if (!is.data.frame(x) || !all(need %in% names(x)))
    fail("Return a data frame with Diet, n, weight_mean, weight_sd for Time == 21.")
  pass("✅ Grouped Day 21 summary OK.")
})
```
...
::: {.callout-note title="Info"}
A histogram shows distribution shape; overlaying Q1/Q3 and Tukey fences makes unusual values visible. Compute quartiles/fences from the Day 21 subset of ChickWeight and add vertical reference lines.
:::

```{webr}
#| exercise: ex_chickweight_hist_iqr
#| exercise.lines: 14
#| echo: false
# Return a ggplot object (no variable) for ChickWeight at Time == 21
```

::: {.hint exercise="ex_chickweight_hist_iqr"}
Subset to the target day inside both the data and the quartile calculations; draw geom_histogram(bins = 20) and then add multiple geom_vline()s at Q1, Q3, and the Tukey bounds.
:::

::: {.solution exercise="ex_chickweight_hist_iqr"}
```{webr}
#| exercise: ex_chickweight_hist_iqr
#| solution: true
ggplot2::ggplot(
  ...
  ggplot2::geom_vline(
    xintercept = c(
      as.numeric(quantile(subset(ChickWeight, Time == 21)$weight, 0.25)),
      as.numeric(quantile(subset(ChickWeight, Time == 21)$weight, 0.75)),
      {
        qs <- quantile(subset(ChickWeight, Time == 21)$weight, c(.25, .75))
        lo <- qs[1] - 1.5 * diff(qs)
        hi <- qs[2] + 1.5 * diff(qs)
        c(lo, hi)
      }
...
gradethis::grade_this({
  ...
  if (!(has_hist && has_vline)) fail("Use `geom_histogram(bins=20)` and add vlines for Q1/Q3 & Tukey bounds (Time == 21).")
  pass("✅ Histogram with IQR fences for ChickWeight Day 21.")
})
```
...
Create a ggplot using mtcars with mpg mapped on x.

Add a histogram layer and choose ~12 bins.

Save the plot object in p_hist.
:::
...
::: {.callout-note title="Info"}
Use the built-in **mtcars** dataset. Make a **boxplot** of **mpg** grouped by the number of cylinders (**cyl**, treated as a categorical variable). Store the plot object in **`p_box`**. Return a **ggplot** object.
:::
...
Build a ggplot from mtcars.

Map cyl (as a factor) to the x-axis and mpg to y.
...
::: {.callout-note title="Info"}
Use the built-in **mtcars** dataset. Make a scatter plot of **wt** (x) vs **mpg** (y) and add a smoother. Store the plot in **`p_scatter`**. Return a **ggplot** object.
:::
...
Build a ggplot from mtcars mapping wt to x and mpg to y.

Add points.

Add a smooth curve (hide the ribbon/SE band).
...
::: {.callout-note title="Info"}
Use the **iris.csv** file created in setup (same columns as the built-in `iris`).  
Read it into an object (e.g., `df`), then compute the **mean Sepal.Length by Species** using **base R** only.  
Store the result in **`mean_sl_by_species`** with columns **`Species`** and **`mean_SL`**.  
Be robust to potential missing values.
:::

```{webr}
#| setup: true
# Prep a CSV the grader expects
...
#| exercise.lines: 4
#| echo: false
mean_sl_by_species <- NULL
```

::: {.hint exercise="ex_aggregate_iris"}

Read the CSV you wrote in setup: df <- read.csv("iris.csv").

Use base aggregate() with a formula interface to group by Species.

Make the mean robust to NAs (think na.rm = TRUE).

Rename the second column to mean_SL.
:::
...
mean_sl_by_species <- aggregate(
  Sepal.Length ~ Species,
  data = df,
  FUN = function(x) mean(x, na.rm = TRUE)
)
names(mean_sl_by_species)[2] <- "mean_SL"
...
#| check: true
gradethis::grade_this({
  # Look in the exercise result environment (important!)
  e <- get0(".envir_result", ifnotfound = parent.frame())
  if (!exists("mean_sl_by_species", envir = e)) fail("Create `mean_sl_by_species`.")
  x <- get("mean_sl_by_species", envir = e)

  if (!is.data.frame(x)) fail("`mean_sl_by_species` should be a data frame.")
  if (!identical(names(x), c("Species", "mean_SL")))
    fail("Rename columns to `Species` and `mean_SL`.")

  # Recompute expected result from iris.csv (NA-robust)
  df <- tryCatch(read.csv("iris.csv"), error = \(err) NULL)
  if (is.null(df)) fail("Couldn't read `iris.csv`.")
  expected <- aggregate(
    Sepal.Length ~ Species,
    data = df,
    FUN = function(v) mean(v, na.rm = TRUE)
  )
  names(expected)[2] <- "mean_SL"

  # Compare after sorting by Species to avoid order issues
  xs <- x[order(as.character(x$Species)), , drop = FALSE]
  es <- expected[order(as.character(expected$Species)), , drop = FALSE]

  same_species <- identical(as.character(xs$Species), as.character(es$Species))
  same_vals <- isTRUE(all.equal(xs$mean_SL, es$mean_SL, tolerance = 1e-8))
  if (!(same_species && same_vals))
    fail("Values don’t match the NA-robust group means from `iris.csv`.")

  pass("✅ Means by species computed (base R, NA-robust).")
})
```
