...
Find $x^*$ such that $P(X <= x^*)$ = 0.95, where  $X ∼ {\chi^2}_{10}$.
...
<figure style="text-align:center">
  <img src="./images/sleep.jpg" style="width:75%">
  <figcaption>Photo by <a href="https://unsplash.com/@mpho_mojapelo?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Mpho Mojapelo</a> on <a href="https://unsplash.com/photos/pink-bell-alarm-clock-showing-210-I84vGUYGUtQ?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
      </figcaption>
</figure>
...
#| check: true
gradethis::grade_this({
  if (!exists("ms_skim", envir=.envir_result)) fail("Assign `ms_skim <- skim(df)` and print it.")
...
#| echo: false
library(ggplot2)
p_hist <- ＿＿ (df, aes( ＿＿ )) + ＿＿ (binwidth = ＿＿ , fill=" ＿＿ ", color="black") 
p_hist
...
#| echo: false
library(ggplot2)
p_scatter <- ＿＿ (df, aes(x= ＿＿ , y = ＿＿ ,  color=" ＿＿ ")) +
   ＿＿ () +
  scale_x_log10() +
...
p_clt_n10 <- ＿＿ (data.frame(m = means_n10), aes(m)) +
  ＿＿ (binwidth = ＿＿ , fill = " ＿＿ " , color = "white") +
   ＿＿ (xintercept = ＿＿ , linetype = 2, color = "red") +
