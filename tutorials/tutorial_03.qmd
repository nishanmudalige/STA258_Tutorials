...
The question below has a dataframe df created from the dataset ggplot2::msleep that contains data about mammal sleep traits. Make a skim summary of df and store it in ms_skim. Print it to see your result!
...
## Q13 — Boxplot with ggplot

Make a boxplot of sleep_total by vore (i.e. mammal's diet) and flip the coords so categories read top-to-bottom, by replacing the ＿＿ with the correct answer. Save the plot to p_box. Fill the boxplot with any color of your choice!
...
ensure_pkgs <- function(pkgs){ miss <- pkgs[!vapply(pkgs, requireNamespace, logical(1), quietly = TRUE)]
  if (length(miss)) webr::install(miss)
  invisible(lapply(pkgs, function(p) library(p, character.only = TRUE))) }
ensure_pkgs(c("ggplot2","dplyr"))
df <- ggplot2::msleep %>% dplyr::filter(!is.na(vore))
...
#| exercise.lines: 5
#| echo: false
p_box <- ＿＿ ::ggplot(df, ggplot2::aes(x = ＿＿ , y = sleep_total)) +
  ggplot2:: ＿＿ (fill = " ＿＿ ", color = "black") +
  ggplot2::＿＿() #for flipping coordinates
p_box 


```

::: {.hint exercise="ms_box"}
Look up how to make boxplots with ggplot.
:::

::: {.solution exercise="ms_box"}
```{webr}
#| exercise: ms_box
#| solution: true
p_box <- ggplot2::ggplot(df, ggplot2::aes(x = vore, y = sleep_total)) +
...
## Q16 — Sample Mean and Sample Standard Deviation
...
## Q17 — Visualizing the Distribution
