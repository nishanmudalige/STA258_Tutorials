---
title: "Tutorial 08 — One-way ANOVA and Multiple Comparisons"
format: live-html
engine: knitr
resources:
...
In the city lifestyle dataset, we want to test whether the **mean happiness score** differs across four regions:
...
- Compute SSB, SSW, `df_between`, `df_within`, MSB, MSW, F, and the p-value
...
Return a named numeric vector: `c(F = Fstat, df1 = df_between, df2 = df_within, p_value = pval)`.
...
$$
\mathrm{SSB} = \sum_i n_i \, (\bar{y}_i - \bar{y})^2
$$

**Within–group sum of squares**
$$
\mathrm{SSW} = \sum_i \sum_j (y_{ij} - \bar{y}_i)^2
$$
...
**Mean squares**
$$
\mathrm{MSB} = \frac{\mathrm{SSB}}{df_{\text{between}}}, \qquad
\mathrm{MSW} = \frac{\mathrm{SSW}}{df_{\text{within}}}
$$
:::
...
SSW <- sum(tapply(y, g, function(x) sum((x - ___(x))^2)))
...
Fstat <- ___ / ___
pval <- pf(___, df_between, df_within, lower.tail = FALSE)

c(F = Fstat, df1 = df_between, df2 = df_within, p_value = pval)
```

::: {.hint exercise="q1_city_anova_manual"}
Use mean for both group and overall means.
df_between = k - 1, df_within = N - k.
MSB = SSB / df_between, MSW = SSW / df_within, then Fstat = MSB / MSW.
:::
...
Fstat <- MSB / MSW
pval  <- pf(Fstat, df_between, df_within, lower.tail = FALSE)
...
exp <- c(F = Fstat, df1 = df_between, df2 = df_within, p_value = pval)
...
fail("Something is off in SSB/SSW, degrees of freedom, or the F to p-value step.")
}
})
...
res <- .result
if (!is.matrix(res) || any(!is.finite(res))) {
fail("Return the p-value matrix from pairwise.t.test(...).")
} else if (all(dim(res) == dim(exp)) &&
max(abs(res - exp)) < 1e-8) {
...
df_between = k - 1, df_within = N - k, MSB = SSB/df_between, MSW = SSW/df_within.
:::
...
fail("Re-check your SSB/SSW, degrees of freedom, and F to p-value computation.")
}
})
...
exp <- pairwise.t.test(sub$Exam_Score,
sub$mot,
p.adjust.method = "none")$p.value
